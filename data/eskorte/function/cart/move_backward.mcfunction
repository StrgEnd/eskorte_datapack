...
execute if score #current_wp temp > #last_cp temp as @e[type=armor_stand,tag=escort_waypoint] if score @s waypoint_id = #target_wp temp run function eskorte:cart/move_to_waypoint

# Stoppen wenn am Checkpoint angekommen
execute unless score #current_wp temp <= #last_cp temp run function eskorte:cart/stop
execute unless score #current_wp temp <= #last_cp temp run tellraw @a[team=defenders] {"text":"Lore am Checkpoint gestoppt!","color":"blue"}
